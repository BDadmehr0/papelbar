...
class MyUserManager(BaseUserManager):
    def create_user(self, phone_number=None, password=None, username=None, **extra_fields):
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(max_length=150, unique=True, null=True, blank=True)  # مخصوص admin
...
class OTP(models.Model):
    ...
    def is_valid(self):
        return timezone.now() <= self.created_at + timedelta(minutes=5)  # OTP 5 دقیقه اعتبار دارد
